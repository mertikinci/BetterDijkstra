...
public class MazeSolver implements InterfaceMazeSolver {
    ...
    public MazeSolver(boolean[][] arr, int searchNo, int heuristicNo, int startX, int startY, int endX, int endY) {
        ...
        if (searchNo<2){ // myalgo or Dijkstra
            if(heuristicNo==0){
    ...
    public Stack<Node> searchPath(boolean algorithm ) {
    ...
    public void setHeuristicChebyshev() {
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[0].length; j++) {
                matrix[i][j].setHeuristic((Math.max(Math.abs(i - endX) , Math.abs(j - endY)) + Math.sqrt(2)-1*
                                                                Math.min(Math.abs(i - endX) , Math.abs(j - endY))));
    ...
    /*public static void main(String args[]){
        ...
        com.mertikinci.mazepuzzle.solver.MazeSolver m = new com.mertikinci.mazepuzzle.solver.MazeSolver(maze,1,123,0,0,30,30);
